function theAxis = mysubplot(nrows, ncols, thisPlot, replace)
%SUBPLOT Create axes in tiled positions.
% slightly modified version of the matlab subplot to move subplots
% closer together...
%
...
elseif(isempty(position))
    ...
    elseif (max(thisPlot) > ncols*nrows)
        ...
        PERC_OFFSET_L = 2*0.045;
        PERC_OFFSET_R = 2*0.0225;
        PERC_OFFSET_B = PERC_OFFSET_L;
        PERC_OFFSET_T = PERC_OFFSET_R;
        if nrows > 2
            PERC_OFFSET_T = 0.95*PERC_OFFSET_T;
            PERC_OFFSET_B = 0.95*PERC_OFFSET_B;
        end
        if ncols > 2
            PERC_OFFSET_L = 0.95*PERC_OFFSET_L;
            PERC_OFFSET_R = 0.95*PERC_OFFSET_R;
        end
